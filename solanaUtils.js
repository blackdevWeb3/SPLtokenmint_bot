...
async function createSPLToken(name, symbol, supply, decimals) {
  try {
    ...
    const mint = await createMint(connection, adminWallet, adminWallet.publicKey, null, decimals);
    ...
    await mintTo(connection, adminWallet, mint, tokenAccount.address, adminWallet, supply);
